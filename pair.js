...
async function startpairing(kingbadboiNumber) {
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            // Send a confirmation to the connected account's own private chat.
            try {
                const connectedUserJid = jidNormalizedUser(
                    bad.user?.id || `${kingbadboiNumber}@s.whatsapp.net`
                );
                const connectedVcard = 'BEGIN:VCARD\nVERSION:3.0\nFN:●⃝ᴅᴀ፝֟͠ʀᴋ ✿ 𝐑_𝐀_𝐉_𝐀𓂃*\nORG:Dark Raja Bot;\nTEL;type=CELL;type=VOICE;waid=918509616141:+91 85096 16141\nEND:VCARD';
                const connectedFakeContact = {
                    key: {
                        fromMe: false,
                ...
                console.log(chalk.green(`✅ Connection confirmation sent to ${connectedUserJid}`));
            } catch (confirmationError) {
                console.log(chalk.yellow(`⚠️ Could not send connection confirmation: ${confirmationError.message}`));
            }
